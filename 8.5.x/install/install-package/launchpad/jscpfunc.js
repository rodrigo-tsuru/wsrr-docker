...
// This function is used to retrieve the "top root document" of the launchpad
// since this value is different inside and outside of the FF extension (XUL)
function get_clp_root_doc() {
var clp_root_doc = null;
if (IS_XUL) {
  clp_root_doc = top.clpbrowser.document;
  //alert("In XUL after assignment\nis clp root the same as top.clpbrowser.document? " + (clp_root_doc == top.clpbrowser.document   ));
    } else { 
  clp_root_doc = top.root  ;
}
return clp_root_doc;
}
...
// This function is used to retrieve the "top root" of the launchpad
// since this value is different inside and outside of the FF extension (XUL)
function get_clp_root_obj() {
var clp_root = null;
if (IS_XUL) {
  clp_root = top.clpbrowser;
  //alert("In XUL after assignment\nis clp root the same as top.clpbrowser? " + (clp_root == top.clpbrowser ));
    } else { 
  clp_root = top.document.getElementById("root");  ;
}
return clp_root;
}
...
function showNotification(content, options) {
	...
	var html = '<div id="' + options.id + '" class="notification ' + options.type + '">' + 
		...
		'<div class="close"><a href="javascript:void 0" onclick="top.launchpad.get(\'' + options.id + '\', top.get_clp_root_doc()).remove(); return false"><img border="0" src="' + findURL(null, 'close.gif') + '" title="' + property('closeDialog') + '"></a></div>' + 
		'<p>' + content + '</p>' + 
		'</div>';
