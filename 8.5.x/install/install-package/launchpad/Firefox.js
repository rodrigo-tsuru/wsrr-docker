...
function secureFileExists(securityFcn,fileName,shouldLogException)
{
	...
	{
		if(typeof shouldLogException == "undefined" || shouldLogException) top.logException(e,fileName); 
	}
	return false;
}
...
function getExternalTopDir() {
	...
	if (top.IS_XUL) {
		var env = Components.classes["@mozilla.org/process/environment;1"].getService(Components.interfaces.nsIEnvironment);
		startingDir = env.get("LaunchPadStartingDir");
	}
	else {
		alert("internal error: invalid reference");
	}
	return startingDir;
}
